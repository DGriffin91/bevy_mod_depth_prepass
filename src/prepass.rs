...
impl ViewPrepassTextures {
    pub fn new(window: &mut Window, images: &mut Assets<Image>, msaa: &Msaa) -> Self {
        let size = Extent3d {
        ...
        image.resize(size);
        Self {
            normals_depth: images.add(image),
        }
    }
}
...
impl Plugin for PrepassPlugin {
    fn build(&self, app: &mut App) {
        app.add_system_to_stage(CoreStage::PreUpdate, window_resized);
...
fn window_resized(
    ...
    prepass_textures: Query<&ViewPrepassTextures>,
    ...
    if let Some(_event) = window_resized_events.iter().last() {
        ...
        if let Some(prepass_texture) = prepass_textures.iter().next() {
